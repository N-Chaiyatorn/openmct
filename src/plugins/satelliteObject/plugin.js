define([
  ...
  function satelliteObjectPlugin(options) {
    ...
    async function getMetadata(namespace) {
      return fetch(`/src/plugins/satelliteObject/metadata/${namespace}Satellite.json`).then(response => {
    ...
    const objectProvider = {
      get: async function (identifier) {
        ...
        if (identifier.key === `${options.namespace}.root`) {
          ...
          return {
            identifier: identifier,
            name: `${satelliteDict.subsystems[identifierKeyArray[1]].hardwares[identifierKeyArray[2]].name}`,
            type: 'folder',
            location: `${options.namespace}:${options.namespace}.${identifierKeyArray[1]}`
          };
        } else {
          const telemetry = satelliteDict.subsystems[identifierKeyArray[1]].hardwares[identifierKeyArray[2]].points.filter(function (telemetryMetadata) {
            return telemetryMetadata.key.split('.')[2,3] === identifierKeyArray[2,3];
          })[0];
          return {
            identifier: identifier,
            name: telemetry.name,
    ...
    const compositionProvider = {
      ...
      load: async function (domainObject) {
        ...
        if (domainObject.type === 'folder' && domainObject.identifier.key.includes('root')) {
          const satelliteSubsystem = subsystemsMap.map((subsystemKey) => {
            return { namespace: options.namespace, key: `${options.namespace}.${subsystemKey}` }
          })
          return satelliteSubsystem
        } else if (domainObject.type === 'folder' && (objectLevel === 2)) {
          const subsystemName = objectKeyArray[1]
          const subsystemHardwares = hardwaresMap[subsystemName].map((hardware) => {
              return { namespace: options.namespace, key: `${domainObject.identifier.key}.${hardware}` }
            })
          return subsystemHardwares
        } else if (domainObject.type === 'folder' && (objectLevel === 3)) {
          const hardwareTelemetry = telemetryMap[domainObject.identifier.key].map((telemetry) => {
              return { namespace: options.namespace, key: `${domainObject.identifier.key}.${telemetry}` }
            })
