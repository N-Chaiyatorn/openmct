define([
  ...
  function fprimeObjectPlugin(options) {
    ...
    async function getMetadata(dataType) {
      return fetch(`/src/plugins/fprimeObject/metadata/${options.namespace}${dataType}Points.json`).then(response => {
    ...
    const objectProvider = {
      get: async function (identifier) { 
        if (identifier.key === `${options.namespace}.root`) {
          ...
          if (packetDict[`${options.namespace}Channels`].points.indexOf(identifier.key) !== -1) {
          ...
          } else if (packetDict[`${options.namespace}Events`].points.indexOf(identifier.key) !== -1) {
            ...
            } if (packetDict[`${options.namespace}Commands`].points.indexOf(identifier.key) !== -1) {
    ...
    const compositionProvider = {
      ...
      load: async function (domainObject) {
        if (domainObject.type === 'folder' && domainObject.identifier.key.includes('root')) {
          return [
            { namespace: options.namespace, key: `${options.namespace}.channels` },
            { namespace: options.namespace, key: `${options.namespace}.events` },
            { namespace: options.namespace, key: `${options.namespace}.commands` }
          ]
